#Script by Georgia Loukatou measuring syllable complexity
...
l = [item.split()[0] for item in l if len(item.split())>0] #list l has all vowels
...
for line in f:
   lastvowel=""
   line=line.strip()
   line1=line.split(";eword") ####list of words per utterance
   for word in line1:
    if len(word)>0:
     i=0 
     nword=nword + 1
     word=word.strip()
     word1=word.split(" ") ####list of phonemes per word
